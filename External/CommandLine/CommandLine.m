    ...
    if (lastParsedOption) {
        for (NSString *key in optionsMatrix) {
            if (![[key description]compare: lastParsedOption] &&
                (![[optionsMatrix valueForKey:key]isEqualToString: CL_OPTION_REQUIRED_WITH_VALUE]
                 || ![[optionsMatrix valueForKey:key]isEqualToString: CL_OPTION_OPTIONAL_WITH_VALUE]
                 )) {
        ...
        while (clPointer < switchEnd) {
            ...
            for (NSString *key in optionsMatrix) {
                if (![[key description]compare: switchString]) {
                    ...
                    found = YES;
                    if (![[optionsMatrix valueForKey: key] isEqualToString: CL_OPTION_REQUIRED_WITH_VALUE]
                        || ![[optionsMatrix valueForKey: key] isEqualToString: CL_OPTION_OPTIONAL_WITH_VALUE]
                        ) {
